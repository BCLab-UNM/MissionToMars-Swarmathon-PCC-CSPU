...
QString GazeboSimManager::stopHectorQuadrotor()
{
//    if (rover_processes.find(rover_name) == rover_processes.end()) return "Could not stop " + rover_name + " rover process since it does not exist.";
...
//    rover_processes.erase(rover_name);
    ...
    hector_quadrotor_processes->terminate();
    hector_quadrotor_processes->waitForFinished();
...
QString GazeboSimManager::startHectorGuadrotor()
{
    QString argument = "roslaunch "+app_root+"/simulation/launch/hector_quadrotor.launch" + ">"+log_root+"rover_processes"+".log";

      hector_quadrotor_processes = new QProcess();
      hector_quadrotor_processes->start("sh", QStringList() << "-c" << argument);

      return "hector_quadrotor process spawned";
}

/**
QString GazeboSimManager::startHectorQradrotor( QString HectorQradrotor_name )
{
  QString argument = "roslaunch "+app_root+"/launch/swarmie.launch name:="+rover_name+">"+log_root+rover_name+".log";
